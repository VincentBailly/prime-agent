...
vi.mock("node:fs", async (importOriginal) => {
	...
	return {
		...
		openSync: new Proxy(actual.openSync, {
			apply(target, thisArg, argArray) {
				const fd = Reflect.apply(target, thisArg, argArray) as number;
				if (ledgerReadTracking.enabled && argArray[1] === "r") ledgerReadTracking.readFds.add(fd);
				return fd;
			},
		}),
		closeSync: new Proxy(actual.closeSync, {
			apply(target, thisArg, argArray) {
				const trackedRead = ledgerReadTracking.readFds.delete(argArray[0] as number);
				const result = Reflect.apply(target, thisArg, argArray);
				if (trackedRead) {
					ledgerReadTracking.count++;
					const afterRead = ledgerReadTracking.afterRead;
					ledgerReadTracking.afterRead = undefined;
					afterRead?.();
				}
